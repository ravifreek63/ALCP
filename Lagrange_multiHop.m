...
collectedDataPerRound = fopen ('Results/ALCP/dataPerRound_ALCP_Hierarchichal.txt', 'w');

% Global Variables %

max_death_Percent = 0.98;
totalMessages = 0;
L = 100;
...
Sn_Energy = ones(n,1) * Energy_init; %Initial Energy for each Sensor Node = 0.5 J.
     ...
     while(1)
        ...
        if(init == 0)
            ...
            numMessagesMatrix (num_it) = numMessages ;
             ...
             fprintf(collectedDataPerRound, '%d\t%d\t%f\t%d\t%d\t%f\t%f\t%f\r\n',  eff_num_it, nodesAlive, Dead_Count/n, numMessagesTransmitted, numClusterHead, averageEnergy, stdDevEnergy, totalEnergyDissipated);
