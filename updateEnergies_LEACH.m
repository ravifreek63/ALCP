...
function [Sn_Energy,Is_Alive, Dead_Count]=  updateEnergies_LEACH(Sn_Energy, Y, Sn, Packet_Transmission_Cost, Packet_Size, Amplification_Energy, D,Min_Energy, death_Percent, BS)
Is_Alive = 1;
Dead_Count = 0 ;
Sensor_Nodes_Number = length(Sn_Energy); 
 for i = 1 : Sensor_Nodes_Number
   if(Sn_Energy(i) > Min_Energy)   % Whenever Energy Is Reduced Below Min_Energy (0.1 Joules Here) The Sensor Node Dies.
    for j = 1 : length(Sn)
        if(Y(i,j) == 1 && i~=j)         %Criteria For A Sensor Node
                Cost = 1.0625*Packet_Size * (Packet_Transmission_Cost + Amplification_Energy * (D(i,j) ^2));
    ...
    if(Y(i,i) == 1)
          Cost = 1.0625 * Packet_Size * (Packet_Transmission_Cost + Amplification_Energy * (getCost(Sn(i,1),Sn(i,2),BS(1,1),BS(1,2)) ^2)); 
          Sn_Energy(i) = max(0,Sn_Energy(i) - Cost) ; 
