function    [Y,value] = solve_p_median_searchHeuristic(Sn_Energy,Sn,Lm,p,D,Packet_Transmission_Cost, Packet_Size, Amplification_Energy, Min_Energy, t, BS)
  ...
  %  InverseCost
   ...
   % X = allocateClusterHeadPositions(B,Ch,p);  
     % Lower Bound Is The Sum Over 'p' smallest values of Bj.
      currentLB = findLB(D,Sn,Lm,Y,t,BS);% sum(B((1:p),2))+ t*sum(Lm); %Calculating the Lower Bound = sum of first p and Lambda.%trace(D * transpose(Y));
  ...
  Y1 = assignClusterHead(Sn,D,Min_Energy,Sn_Energy,Y);
% Y
 value = zeros(1,1);
 value  = trace((Y1 * transpose (D)));
 %value
